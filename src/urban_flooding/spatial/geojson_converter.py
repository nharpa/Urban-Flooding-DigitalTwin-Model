...
def calculate_pipe_capacity(diameter_mm: float, slope: float, material: str = "RC") -> float:
    ...
    velocity = (1.0 / manning_n) * (hydraulic_radius ** (2.0/3.0)) * (slope ** 0.5)
...
def aggregate_pipes_with_location(pipes_file: str) -> Dict[str, Dict]:
    ...
    for feature in pipes_data['features']:
        ...
        diameter = props.get('DIAMETER', 0); length = props.get('PIPE_LENGTH', 0)
        grade = props.get('GRADE', 0); material = props.get('MATERIAL', 'RC')
                ...
                for coord in coords:
                    if isinstance(coord, (list, tuple)) and len(coord) >= 2:
                        try:
                            ...
                            continue
                if valid_coords:
                    subcatchment_data[subcatchment]['all_coordinates'].extend(valid_coords)
    ...
    for subcatch, data in subcatchment_data.items():
        ...
        main_pipes = sorted(pipes, key=lambda x: x['diameter'], reverse=True)[:3]
...
def extract_catchments_with_geometry(catchments_file: str) -> Dict[str, Dict]:
    ...
    for idx, feature in enumerate(catchments_data['features']):
        ...
        bounds = get_polygon_bounds(geom); center = get_polygon_center(geom)
...
def spatial_match_catchments(subcatchment_pipes: Dict, catchment_areas: Dict, default_C: float = 0.6) -> List[Dict]:
    ...
    for subcatch_id, pipe_info in subcatchment_pipes.items():
        ...
        best_match = None; best_match_score = 0
        closest_distance = float('inf'); closest_match = None
        for area_name, area_info in catchment_areas.items():
            area_center = area_info.get('center')
            if not area_center:
                continue
            distance = calculate_distance(pipe_center[0], pipe_center[1], area_center['lon'], area_center['lat'])
            if distance < closest_distance:
                closest_distance = distance; closest_match = area_info
            area_bounds = (
                area_info['bounds']['min_lon'], area_info['bounds']['min_lat'],
                area_info['bounds']['max_lon'], area_info['bounds']['max_lat']
            )
            lon_overlap = min(pipe_info['bounds']['max_lon'], area_info['bounds']['max_lon']) - max(pipe_info['bounds']['min_lon'], area_info['bounds']['min_lon'])
            lat_overlap = min(pipe_info['bounds']['max_lat'], area_info['bounds']['max_lat']) - max(pipe_info['bounds']['min_lat'], area_info['bounds']['min_lat'])
            if lon_overlap > 0 and lat_overlap > 0:
                overlap_area = lon_overlap * lat_overlap
                pipe_area = (pipe_info['bounds']['max_lon'] - pipe_info['bounds']['min_lon']) * (pipe_info['bounds']['max_lat'] - pipe_info['bounds']['min_lat'])
                if pipe_area > 0:
                    score = (overlap_area / pipe_area) * (1.0 / (1.0 + distance * 10))
                    if score > best_match_score:
                        best_match_score = score; best_match = area_info
        ...
        if best_match and best_match_score > 0.01:
            ...
            catchment_record['basin_name'] = closest_match.get('basin_name', '')
            catchment_record['area_type'] = closest_match.get('type', '')
            catchment_record['matched_catchment'] = closest_match['name']
            catchment_record['match_distance_km'] = round(closest_distance * 111, 2)
            catchment_record['match_score'] = round(1.0 / (1.0 + closest_distance * 10), 3)
            ...
            width_km = (pipe_bounds['max_lon'] - pipe_bounds['min_lon']) * 111.0
            height_km = (pipe_bounds['max_lat'] - pipe_bounds['min_lat']) * 111.0
            estimated_area = max(width_km * height_km * 1.5, pipe_info['total_length_m'] / 1000.0 * 0.5)
...
def main():  # pragma: no cover - convenience script
    ...
    for i, (subcatch, info) in enumerate(list(subcatchment_pipes.items())[:3]):
        ...
        print(f"     - Center point: [{info['center']['lon']}, {info['center']['lat']}]")
    ...
    print(f"   Found {len(catchment_areas)} catchment areas (total {len(catchment_areas)} with valid area)")
    print("\n3. Executing spatial matching...")
    matched_data = spatial_match_catchments(subcatchment_pipes, catchment_areas)
    ...
    overlap_count = sum(1 for c in matched_data if c.get('match_type') == 'overlap')
    nearest_count = sum(1 for c in matched_data if c.get('match_type') == 'nearest')
    estimated_count = sum(1 for c in matched_data if c.get('area_estimated', False))
    ...
    if overlap_count + nearest_count > 0:
        match_scores = [c['match_score'] for c in matched_data if 'match_score' in c and c['match_score'] > 0]
        if match_scores:
            avg_score = np.mean(match_scores); max_score = np.max(match_scores)
    ...
    for i, record in enumerate(matched_data[:5]):
        ...
        print(f"   - Location: [{record['location']['center']['lon']:.4f}, {record['location']['center']['lat']:.4f}]")
        if 'matched_catchment' in record:
            match_type = record.get('match_type', 'unknown')
            if match_type == 'overlap':
                print(f"   - Matched to: {record['matched_catchment']} (overlap match, score: {record['match_score']})")
            elif match_type == 'nearest':
                distance = record.get('match_distance_km', 'N/A')
                confidence = record.get('area_confidence', 'N/A')
                print(f"   - Matched to: {record['matched_catchment']} (nearest match, distance: {distance}km, confidence: {confidence})")
        else:
