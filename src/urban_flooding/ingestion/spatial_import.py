...
def import_spatial_catchments(data: list, db: FloodingDatabase) -> Tuple[int, int, int]:
    ...
    for catchment in data:
        try:
            if 'C' in catchment: catchment['C'] = float(catchment['C'])
            if 'A_km2' in catchment: catchment['A_km2'] = float(catchment['A_km2'])
            if 'Qcap_m3s' in catchment: catchment['Qcap_m3s'] = float(catchment['Qcap_m3s'])
            if 'total_pipe_length_m' in catchment: catchment['total_pipe_length_m'] = float(catchment['total_pipe_length_m'])
            if 'max_pipe_diameter_mm' in catchment: catchment['max_pipe_diameter_mm'] = float(catchment['max_pipe_diameter_mm'])
            if 'pipe_count' in catchment: catchment['pipe_count'] = int(catchment['pipe_count'])
            if 'location' in catchment and catchment['location']:
                location = catchment['location']
                if 'center' in location:
                    center = location['center']
                    if isinstance(center, dict) and 'lat' in center and 'lon' in center:
                        center['lat'] = float(center['lat']); center['lon'] = float(center['lon'])
                    elif isinstance(center, list) and len(center) == 2:
                        center[0] = float(center[0]); center[1] = float(center[1])
                if 'bounds' in location and location['bounds']:
                    bounds = location['bounds']
                    for k in ['min_lon', 'min_lat', 'max_lon', 'max_lat']:
                        if k in bounds: bounds[k] = float(bounds[k])
            existing = db.get_catchment(catchment['catchment_id'])
            if existing:
                print(f"Update: {catchment['catchment_id']} - {catchment.get('name')}"); updated_count += 1
            else:
                print(f"Import: {catchment['catchment_id']} - {catchment.get('name')}"); imported_count += 1
            db.save_catchment_full(catchment)
        except Exception as e:  # pragma: no cover
            print(f"Error processing {catchment.get('catchment_id')}: {e}"); skipped_count += 1
...
def create_design_rainfall_events(db: FloodingDatabase) -> int:
    ...
    for event in events:
        ...
        if 'total_rainfall_mm' not in event: event['total_rainfall_mm'] = float(sum(event['rain_mmhr']))
        if 'peak_intensity_mmhr' not in event: event['peak_intensity_mmhr'] = float(max(event['rain_mmhr']))
        if 'duration_hours' not in event: event['duration_hours'] = float(len(event['rain_mmhr']))
        try:
            db.save_rainfall_event(**event); print(f"Created rainfall event: {event['name']}")
        except Exception as e:  # pragma: no cover
...
def run_risk_assessment(db: FloodingDatabase, num_samples: int = 10, event_id: str = "design_10yr"):
    catchments = db.list_catchments(); catchments_sorted = sorted(catchments, key=lambda x: x.get('Qcap_m3s', float('inf')))
    selected = catchments_sorted[:num_samples]
    event = db.get_rainfall_event(event_id)
    if not event:
        print(f"Rainfall event not found: {event_id}"); return 0, 0
    print(f"\nUsing rainfall event: {event['name']}")
    print(f"Running risk assessment for {len(selected)} catchments...")
    simulation_count = 0; high_risk_count = 0
    for catchment in selected:
        ...
        if results['max_risk'] > 0.7:
            risk_level = "High"; high_risk_count += 1
        elif results['max_risk'] > 0.3:
            risk_level = "Medium"
        print(f"  {catchment['name']}: risk={results['max_risk']:.3f} ({risk_level})")
        if 'location' in catchment and catchment['location']:
            center = catchment['location'].get('center', {})
            if center:
                print(f"    Location: [{center.get('lon', 'N/A'):.4f}, {center.get('lat', 'N/A'):.4f}]")
...
def generate_spatial_risk_report(db: FloodingDatabase):
    ...
    print(f"  Average drainage capacity: {stats.get('avg_capacity_m3s', 0):.3f} m³/s")
    print(f"  Total pipe network length: {stats.get('total_pipe_length_km', 0):.2f} km")
    ...
    if high_risk_sims:
        ...
        for sim in high_risk_sims:
            risk_by_catchment.setdefault(sim['catchment_id'], []).append(sim['max_risk'])
        for cid, risks in list(risk_by_catchment.items())[:5]:
            catchment = db.get_catchment(cid)
            if catchment:
                avg_risk = sum(risks) / len(risks); max_risk = max(risks)
                ...
                if 'location' in catchment and catchment['location']:
                    center = catchment['location'].get('center', {})
                    if center:
                        print(f"  Location: longitude={center.get('lon', 'N/A'):.4f}, latitude={center.get('lat', 'N/A'):.4f}")
                if 'pipe_count' in catchment:
                    print(f"  Pipe count: {catchment.get('pipe_count', 0)}")
                    print(f"  Maximum pipe diameter: {catchment.get('max_pipe_diameter_mm', 0)} mm")
    ...
    for min_cap, max_cap, label in capacity_ranges:
        count = len(db.get_catchments_by_capacity(min_cap, max_cap if max_cap != float('inf') else None))
        if count > 0:
...
def main():  # pragma: no cover
    ...
    try:
        catchments_data = load_spatial_data(); print(f"   Loaded {len(catchments_data)} catchment area records")
    except FileNotFoundError:
        print("   Error: catchments_spatial_matched.json not found")
        print("   Please run geojson_converter_spatial.py first to generate data"); return
    print("\n3. Importing catchment areas to MongoDB...")
    imported, updated, skipped = import_spatial_catchments(catchments_data, db)
    print(f"   New imports: {imported}, Updates: {updated}, Skipped: {skipped}")
    print("\n4. Creating design rainfall events...")
    event_count = create_design_rainfall_events(db); print(f"   Created {event_count} rainfall events")
    print("\n5. Running risk assessment simulations...")
    sim_count, high_risk = run_risk_assessment(db, num_samples=10, event_id="design_10yr")
    print(f"   Completed {sim_count} simulations, found {high_risk} high risk areas")
    ...
    nearby = db.find_catchments_by_location(115.857, -31.955, max_distance_km=2)
