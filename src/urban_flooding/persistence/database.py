...
class FloodingDatabase:
    ...
    def __init__(self, connection_uri: str = None, db_name: str = "urban_flooding_dt"):
        self.uri = connection_uri or os.getenv("MONGODB_URI", "mongodb://localhost:27017/")
    ...
    def _create_indexes(self):
        ...
        self.simulations.create_index([("catchment_id", ASCENDING), ("created_at", DESCENDING)])
    ...
    def save_catchment_full(self, catchment_data: Dict) -> str:
        ...
        self.catchments.replace_one({"catchment_id": catchment_data["catchment_id"]}, catchment_data, upsert=True)
    ...
    def save_catchment(self, catchment_id: str, name: str, C: float, A_km2: float, Qcap_m3s: float, **kwargs) -> str:
        doc = {"catchment_id": catchment_id, "name": name, "C": C, "A_km2": A_km2, "Qcap_m3s": Qcap_m3s, "updated_at": datetime.utcnow()}
        existing = self.catchments.find_one({"catchment_id": catchment_id})
        doc["created_at"] = existing.get("created_at", datetime.utcnow()) if existing else datetime.utcnow()
        for k, v in kwargs.items():
            if v is not None:
                doc[k] = v
        self.catchments.replace_one({"catchment_id": catchment_id}, doc, upsert=True)
        return catchment_id
    ...
    def find_catchments_by_location(self, lon: float, lat: float, max_distance_km: float = 10.0) -> List[Dict]:
        ...
        query = {"location.center.lon": {"$gte": lon - degree_offset, "$lte": lon + degree_offset}, "location.center.lat": {"$gte": lat - degree_offset, "$lte": lat + degree_offset}}
    ...
    def find_catchments_in_bounds(self, min_lon: float, min_lat: float, max_lon: float, max_lat: float) -> List[Dict]:
        query = {"location.center.lon": {"$gte": min_lon, "$lte": max_lon}, "location.center.lat": {"$gte": min_lat, "$lte": max_lat}}
        return list(self.catchments.find(query, {"_id": 0}))

    def get_catchment_with_pipes(self, catchment_id: str) -> Optional[Dict]:  # alias
    ...
    def save_simulation(self, simulation_id: str, catchment_id: str, rain_mmhr: List[float], timestamps_utc: List[str], C: float, A_km2: float, Qcap_m3s: float, series: List[Dict], max_risk: float, k: float = 8.0, **kwargs) -> str:
        doc = {"simulation_id": simulation_id, "catchment_id": catchment_id, "rain_mmhr": rain_mmhr, "timestamps_utc": timestamps_utc, "C": C, "A_km2": A_km2, "Qcap_m3s": Qcap_m3s, "k": k, "series": series, "max_risk": max_risk, "created_at": datetime.utcnow()}
    ...
    def get_statistics_with_spatial(self) -> Dict:
        pipeline = [{"$group": {"_id": None, "total_catchments": {"$sum": 1}, "total_area_km2": {"$sum": "$A_km2"}, "avg_capacity_m3s": {"$avg": "$Qcap_m3s"}, "total_pipe_length_km": {"$sum": {"$divide": ["$total_pipe_length_m", 1000]}}, "total_pipes": {"$sum": "$pipe_count"}, "with_location": {"$sum": {"$cond": [{"$ne": ["$location", None]}, 1, 0]}}, "estimated_areas": {"$sum": {"$cond": [{"$eq": ["$area_estimated", True]}, 1, 0]}}}}]
        results = list(self.catchments.aggregate(pipeline))
        if results:
            stats = results[0]; stats.pop("_id"); return stats
    ...
    def get_simulations_by_catchment(self, catchment_id: str, limit: int = 10, skip: int = 0) -> List[Dict]:
        cursor = self.simulations.find({"catchment_id": catchment_id}, {"_id": 0}).sort("created_at", DESCENDING).skip(skip).limit(limit)
        return list(cursor)

    def get_high_risk_simulations(self, risk_threshold: float = 0.7, limit: int = 20) -> List[Dict]:
        cursor = self.simulations.find({"max_risk": {"$gte": risk_threshold}}, {"_id": 0}).sort("max_risk", DESCENDING).limit(limit)
        return list(cursor)

    def get_simulations_by_date_range(self, days_back: int = 7, limit: int = 100) -> List[Dict]:
        from datetime import timedelta
        cutoff_date = datetime.utcnow() - timedelta(days=days_back)
        cursor = self.simulations.find({"created_at": {"$gte": cutoff_date}}, {"_id": 0}).sort("created_at", DESCENDING).limit(limit)
        return list(cursor)

    def save_rainfall_event(self, event_id: str, name: str, rain_mmhr: List[float], timestamps_utc: List[str], **kwargs) -> str:
        doc = {"event_id": event_id, "name": name, "rain_mmhr": rain_mmhr, "timestamps_utc": timestamps_utc, "created_at": datetime.utcnow()}
        if rain_mmhr:
        ...
        for k, v in kwargs.items():
            if v is not None:
                doc[k] = v
        self.rainfall_events.replace_one({"event_id": event_id}, doc, upsert=True)
        return event_id
